...
export function Sidebar({ className, isOpen, onClose }) {
  ...
  return (
    <aside 
      className={cn(
        "flex flex-col w-64 bg-slate-50/70 border-r border-slate-200/60 h-screen shrink-0 transition-transform duration-300",
        "lg:translate-x-0 lg:static lg:flex lg:h-screen",
        "fixed inset-y-0 left-0 z-50 shadow-xl lg:shadow-none bg-white lg:bg-slate-50/70",
        isOpen ? "translate-x-0" : "-translate-x-full",
        className
      )}
    >
      {/* Brand Header */}
      <div className="h-14 border-b border-slate-200/60 flex items-center justify-between px-6 gap-2 bg-white shrink-0">
      ...
      <div className="p-4 border-t border-slate-200/60 bg-white/50 space-y-3 shrink-0">
        <NavLink
          to="/notifications"
          onClick={onClose}
          className={({ isActive }) =>
